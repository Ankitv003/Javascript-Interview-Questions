...
function sortByDuration(data, flightLength){
    ...
    const podcastDuration=data.sort((podcastA,podcastB)=>{
        return (flightLength<=60)? podcastA.duration - podcastB.duration: podcastB.duration - podcastA.duration
    })
       podcastDuration.forEach((podcast, index)=>{
        console.log(`${index+1}. ${podcast.title}, ${podcast.duration} minutes`)
    })
}

sortByDuration(podcasts, 6);
